...
static Geodesic makeLightlikeGeodesic(Tensor pos, Tensor dir, Tetrad tetrad) {
    ...
    Tensor *x_v = Tensor_mul_scalar(&tetrad.v[1], normalizeDir->data[0]);
    Tensor *y_v = Tensor_mul_scalar(&tetrad.v[2], normalizeDir->data[1]);
    Tensor *z_v = Tensor_mul_scalar(&tetrad.v[3], normalizeDir->data[2]);
