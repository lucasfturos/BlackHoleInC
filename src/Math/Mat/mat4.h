...
//! Util
static Mat4 Mat4_create(Vec4 row0, Vec4 row1, Vec4 row2, Vec4 row3) {
    return (Mat4){.v = {row0, row1, row2, row3}};
}

static void UNUSED Mat4_print(Mat4 m) {
    printf("Matrix:\n");
...
//! Mat4 Operations
static Vec4 UNUSED Mat4_mul_vec4(Mat4 m, Vec4 v) {
    return Vec4_create(Vec4_dot(m.v[0], v), Vec4_dot(m.v[1], v),
                       Vec4_dot(m.v[2], v), Vec4_dot(m.v[3], v));
}

static Mat4 UNUSED Mat4_normalize(Mat4 m) {
